...
public class HiloCanal extends Thread {
    ...
    private FFmpegBuilder construirFFmpegBuilderMP4(String input, String out) {
        return new FFmpegBuilder()
                ...
                .setVideoFrameRate(config.getFps()) // Fps
                .setVideoResolution(config.getRes_h(), config.getRes_w()) // Establece la resolución del video a 680x480
                .setVideoBitRate(config.getVbr()) // Establece la velocidad de bits del video a 250Kbps
                //Configuracion de Audio del video
